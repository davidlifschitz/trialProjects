...
public class MyDocumentStoreImplTest {
    ...
    public void init() throws Exception {
        ...
        this.txt4 = "the text of doc4: how much wood would a woodchuck chuck...";
    }
    ...
    public void testGetTxtDocAsPdf(){
        ...
        //TODO allowing for student following old API comment. To be changed for stage 2 to insist on following new comment.
        assertTrue(returned == 0 || returned == this.txt1.hashCode());
        String text = Utils.pdfDataToText(store.getDocumentAsPdf(this.uri1));
        assertEquals("failed to return correct pdf text",this.txt1,text);
    }
    ...
    public void testGetPdfDocAsPdf(){
        ...
        assertTrue(returned == 0 || returned == this.pdfTxt1.hashCode());
        String text = Utils.pdfDataToText(store.getDocumentAsPdf(this.uri1));
        assertEquals("failed to return correct pdf text",this.pdfTxt1,text);
    }
    ...
    @Test 
    public void testUndoAPutWithASpecificURI()
    {
       DocumentStoreImpl dsi = createStoreAndPutOne();
       try {
        dsi.undo(this.uri2);  
    ...
    @Test 
    public void testUndoMultiplePuts()
    {
       DocumentStoreImpl dsi = createStoreAndPutOne();
       dsi.undo();
       assertTrue(dsi.stackImpl.peek() == null);
    ...
    @Test 
    public void testUndoForDeleteALL()
    {
        DocumentStoreImpl store = this.createStoreAndPutMultipleDocsWithTheSameWord();
        Set<URI> docURIs = store.deleteAll("this");
        assertTrue(store.stackImpl.size() == 5);
        store.undo();
        assertTrue(store.stackImpl.size() == 4);
    }
    @Test
    public void testUndoForDeleteAllWithPrefix()
    {
        DocumentStoreImpl store = this.createStoreAndMultipleWordsValuesInDifferentWordsFromOnePrefix();
        Set<URI> docURIs = store.deleteAllWithPrefix("to");
        int size = store.stackImpl.size();
        assertTrue(size == 5);
        store.undo();
        size = store.stackImpl.size();
        assertTrue(size == 4);
    }
    @Test 
    public void testUndoWithURIForDeleteALL()
    {
        DocumentStoreImpl store = this.createStoreAndPutMultipleDocsWithTheSameWord();
        Set<URI> docURIs = store.deleteAll("this");
        assertTrue(store.stackImpl.size() == 5);
    ...
    @Test
    public void TestUndoWithURIForDeleteAllWithPrefix()
    {
        DocumentStoreImpl store = this.createStoreAndMultipleWordsValuesInDifferentWordsFromOnePrefix();
        Set<URI> docURIs = store.deleteAllWithPrefix("to");
        int size = store.stackImpl.size();
        ...
        assertTrue(store.stackImpl.size() == 5);
        CommandSet cS = (CommandSet) store.stackImpl.pop();
